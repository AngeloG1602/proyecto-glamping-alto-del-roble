    ...
    <>
      ...
      <div className="contenedor space-y-12 pb-12 md:space-y-16 md:pb-16">
        {EXTRAS.map((extra, i) => (
          ...
          >
            <div
              className={`group relative aspect-[4/3] overflow-hidden rounded-tarjeta bg-arena shadow-sutil ${i % 2 === 1 ? "md:order-2" : ""}`}
            >
              <Foto
                ...
                prioridad={i === 0}
                className="transition-transform duration-[1400ms] ease-salida group-hover:scale-[1.05]"
              />
            </div>
            <div>
        ...
        <ol className="grid gap-6 md:grid-cols-3">
          {PASOS.map(({ icono: Icono, texto }, i) => (
            <li
              ...
              data-revelar
              style={retraso(i, 120)}
              className="flex gap-4 rounded-tarjeta bg-hueso p-6"
            >
